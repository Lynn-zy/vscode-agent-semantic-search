...
这是一个为 VS Code 设计的桥接扩展插件，旨在将工作区代码语义检索工具（`semantic_search`）开放给所有智能体（Agent），让使用第三方或兼容模型（如 DeepSeek、Gemini、Claude 等）的 Agent 能够自主调用工作区语义代码搜索能力。
...
2. **多态数据适配与防幻觉降级（Graceful Degradation）**：
   ...
   - 当向量索引尚未就绪或检索无结果时，自动附带明确的排查与降级建议，引导 Agent 立即使用 `grep_search` 或 `file_search` 兜底，避免陷入工具调用死循环。

3. **工作区索引一键构建与管理**：
   - 状态栏右下角入口：实时展示语义检索状态（就绪 / 未就绪 / 异常）。
   - 命令面板（Command Palette）：
     - `语义搜索：构建工作区代码库索引`（触发 Copilot 建立本地/远程向量索引）
...
4. **高度可配置**：
   ...
   - `semanticSearch.showDegradationHints`：是否向模型附带降级操作指引。
...
2. 按 `F5` 启动调试窗口；
3. 在弹出的新 VS Code 窗口中开启 Chat，即可测试 Agent 自主调用 `semantic_search`。
